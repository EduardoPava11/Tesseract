...
// ============================================================================
// APP ADDITION (not part of the parity-verified port above):
// boxReduceRGB — area box-reduce of the debayered native-square RGB to the
// active rung resolution S. Integer floor boundaries lo = o*N/S,
// hi = (o+1)*N/S partition [0,N) exactly (DNGPipeline.boxBounds mirrors this
// math and is unit-tested). Boxes are 2 or 3 px wide when N/S is fractional
// (3024/1024 = 2.95): every source pixel lands in exactly one box.
// ============================================================================

kernel void boxReduceRGB(device const float *inRGB  [[buffer(0)]],
                         device float *outRGB       [[buffer(1)]],
                         constant int2 &dims        [[buffer(2)]],   // (N, S)
                         uint2 gid [[thread_position_in_grid]])
{
    const int N = dims.x;
    const int S = dims.y;
    if (int(gid.x) >= S || int(gid.y) >= S) return;

    const int y0 = int(gid.y) * N / S;
    const int y1 = (int(gid.y) + 1) * N / S;
    const int x0 = int(gid.x) * N / S;
    const int x1 = (int(gid.x) + 1) * N / S;

    float3 acc = float3(0.0f);
    for (int y = y0; y < y1; ++y) {
        device const float *row = inRGB + (y * N + x0) * 3;
        for (int x = x0; x < x1; ++x) {
            acc += float3(row[0], row[1], row[2]);
            row += 3;
        }
    }
    const float inv = 1.0f / float(max((y1 - y0) * (x1 - x0), 1));
    device float *o = outRGB + (gid.y * uint(S) + gid.x) * 3;
    o[0] = acc.x * inv;
    o[1] = acc.y * inv;
    o[2] = acc.z * inv;
}
