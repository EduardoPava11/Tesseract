...
// face's OKLab distribution; complements 128..255 are generated,
// never designed. Face pixels quantize to primaries only; the
// background collapses to index 255 = comp(centroid).
//
// Port of spec/quantization/DyadPalette.hs — the Haskell spec is
// authoritative (axioms DY1–DY7). Weighted statistics are the one
...
enum DyadPalette {
    ...
    static func complement(of rgb: (UInt8, UInt8, UInt8)) -> (UInt8, UInt8, UInt8) {
        srgb8(from: chromaClamp(comp(oklab(fromSRGB8: rgb))))
    }
    ...
    /// The full DYAD table, laid out so that T[255−i] = complement(of: T[i]).
    static func table(stats: Stats) -> [(UInt8, UInt8, UInt8)] {
        let prims = primaries(stats: stats)
        return prims + prims.reversed().map { complement(of: $0) }
    }
